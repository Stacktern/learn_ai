...
x_z = (x - mean) / std`,
  ...
  {
    id: 'data-science-6',
    title: 'Data Science VI · Projects',
    description: 'Three hands-on end-to-end projects from the book — house price prediction, COVID-19 time series, and pneumonia diagnosis with CNNs — with interactive simulators at every step.',
    icon: 'FolderGit2',
    lessons: [
      {
        id: 'hp-overview',
        number: 116,
        title: '116. House Price Prediction — Project Overview',
        subtitle: 'A regression project on the Kaggle House Prices dataset — Random Forest and SVM, plus an invitation to push the MSE lower.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '5 mins',
        difficulty: 'Beginner',
        objectives: [
          'State the task (regression on 1,460 × 81 Kaggle house data)',
          'Outline the seven-step pipeline from import to evaluation',
          'Name the two baseline models the book uses',
        ],
        concepts: [
          {
            title: 'The project',
            paragraphs: [
              ...
              'We will start by importing the libraries: pandas, numpy, sklearn.model_selection.train_test_split, sklearn.ensemble.RandomForestRegressor, sklearn.svm.SVR, sklearn.metrics.mean_squared_error, sklearn.pipeline.Pipeline, sklearn.preprocessing.StandardScaler / OneHotEncoder, sklearn.compose.ColumnTransformer, sklearn.impute.SimpleImputer.',
            ],
            analogy: {
              concept: 'The pipeline',
              metaphor: 'A cooking recipe — prep the data (clean, encode, scale), train the model, taste the result.',
              why: 'Most ML projects collapse without a clean pipeline; this book makes that pipeline explicit.',
            },
          },
          {
            ...
            paragraphs: [
              '1. Import libraries.',
              '2. Load dataset.',
              '3. EDA (shape, describe, missing values).',
              '4. Preprocess (impute, scale, one-hot encode).',
              '5. Split train / test.',
              '6. Fit Random Forest and SVR models.',
              '7. Evaluate with mean squared error.',
            ],
          },
          {
            title: 'Results from the book',
            paragraphs: [
              'Random Forest MSE: 857,641,621.',
              'SVM MSE: 7,854,595,775.',
              'The book says: <em>"I tried to make this first project as simple as possible. Now your job is to implement other machine learning models and get the MSE. Do some more EDA and try to get a better MSE value."</em>',
            ],
          },
        ],
        glossary: [
          { term: 'MSE', definition: 'Mean Squared Error — the average of (yᵢ − ŷᵢ)². The book\'s evaluation metric.' },
          { term: 'Pipeline', definition: 'A scikit-learn object that chains preprocessing and an estimator into a single fit / predict workflow.' },
        ],
        realWorldExample: {
          scenario: 'A real-estate startup needs to value houses from their features',
          application: 'A regression model on the Kaggle House Prices dataset (1,460 rows × 81 columns) gives a starting point — Random Forest outperforms linear SVR out of the box.',
          impact: 'Even a baseline gives the sales team an objective reference value to compare to a human appraisal.',
        },
        playground: {
          type: 'hp-overview',
          title: 'Seven-step pipeline at a glance',
          instructions: 'Read the chart of seven steps on the left. The right panel shows the dataset shape and the two models the book uses.',
          expectedOutcome: 'Be able to recite the seven steps and the two baseline MSEs.',
        },
        quiz: [
          {
            id: 'hpo-q1',
            question: 'The Kaggle House Prices dataset has:',
            options: ['(1460, 81)', '(1000, 20)', '(5000, 100)', '(800, 50)'],
            answerIndex: 0,
            explanation: 'The book: train_data.shape (1460, 81) — 1,460 rows, 81 columns.',
          },
          {
            id: 'hpo-q2',
            question: 'Which model did the book find better?',
            options: ['SVM', 'Random Forest', 'Both equal', 'Neither — the book skipped evaluation'],
            answerIndex: 1,
            explanation: 'Random Forest MSE = 857M vs SVR MSE = 7.85B — RF is much better.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, identify the dataset size from the right panel.',
          hint: 'The book reports shape (1460, 81).',
          expectedAnswer: '1,460 rows × 81 columns.',
        },
        keyTakeaways: [
          'House price prediction is a regression task on the Kaggle House Prices dataset (1,460 × 81).',
          'The book\'s seven-step pipeline: import → load → EDA → preprocess → split → fit → evaluate.',
          'The book\'s baselines: Random Forest (MSE 857M) and SVR (MSE 7.85B) — RF is the better starting point.',
        ],
      },
      {
        id: 'hp-eda',
        number: 117,
        title: '117. House Price Prediction — Data Loading and EDA',
        subtitle: 'Load the dataset, inspect its shape, check for missing values, and visualise the target distribution.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '5 mins',
        difficulty: 'Beginner',
        objectives: [
          'Load a CSV with pandas and inspect it',
          'Use shape, describe, and isnull().sum() for EDA',
          'Visualise the SalePrice distribution with a histogram + KDE',
        ],
        concepts: [
          {
            title: 'Loading the dataset',
            paragraphs: [
              'Now we load the dataset. # Load the dataset. train_data = pd.read_csv("/content/train (1).csv"). print("Columns in the dataset:", train_data.columns).',
              'After loading we will see something like this. Let\'s look at the dataset. train_data.head(). Let\'s look at the shape of the data. train_data.shape (1460, 81). So we can see that it has 1460 rows and 81 columns.',
              'Let\'s describe the data to get some statistical data from the dataset. train_data.describe().',
            ],
          },
          {
            title: 'Missing data and the target distribution',
            paragraphs: [
              'Now let\'s deal with missing values. # Check for missing data. missing_data = train_data.isnull().sum(). missing_data = missing_data[missing_data > 0].sort_values(ascending=False).',
              'Time for some EDA. # Exploratory Data Analysis (EDA) and Visualization. # Distribution of SalePrice (Target Variable). plt.figure(figsize=(10, 6)). sns.histplot(train_data["SalePrice"], kde=True). plt.title("Distribution of Sale Prices"). plt.xlabel("Sale Price"). plt.ylabel("Frequency"). plt.show().',
            ],
          },
        ],
        glossary: [
          { term: 'EDA', definition: 'Exploratory Data Analysis — inspecting the dataset to find missing values, distributions, and outliers before modeling.' },
        ],
        realWorldExample: {
          scenario: 'A Kaggle-style house price workflow',
          application: 'The same EDA pattern (load → shape → describe → isnull → distribution) is the standard opening of any tabular ML project.',
          impact: 'EDA catches errors (negative ages, impossibly high prices) before they corrupt a model.',
        },
        playground: {
          type: 'hp-eda',
          title: 'Top missing values by feature',
          instructions: 'Drag the Resample-seed slider to redraw the chart. Notice how the order of features changes — the relative gap is what matters.',
          expectedOutcome: 'Be able to identify features with many missing values and understand that categorical features like PoolQC and Alley dominate the missing-value list.',
        },
        quiz: [
          {
            id: 'hpe-q1',
            question: 'train_data.shape returns:',
            options: ['A tuple (rows, columns)', 'A list of column names', 'A list of dtypes', 'A pandas Series'],
            answerIndex: 0,
            explanation: 'shape returns a tuple (n_rows, n_columns).',
          },
          {
            id: 'hpe-q2',
            question: 'train_data.isnull().sum() returns:',
            options: ['The mean of numeric columns', 'The count of NaN values per column', 'The unique values in each column', 'The total number of rows'],
            answerIndex: 1,
            explanation: 'isnull().sum() gives the number of NaN entries in each column.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, which two features have the highest missing counts?',
          hint: 'PoolQC and MiscFeature are the typical worst offenders.',
          expectedAnswer: 'PoolQC and MiscFeature (the book uses them as the highest missing-value examples).',
        },
        keyTakeaways: [
          'EDA: load with read_csv, inspect with head() / shape / describe() / isnull().sum().',
          'The House Prices dataset has 1,460 rows × 81 columns.',
          'sns.histplot with kde=True reveals the SalePrice distribution before any modelling.',
        ],
      },
      {
        id: 'hp-preprocess',
        number: 118,
        title: '118. House Price Prediction — Preprocessing Pipeline',
        subtitle: 'Impute missing values, scale numerics, one-hot encode categoricals, and stitch the two together with a ColumnTransformer.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '6 mins',
        difficulty: 'Intermediate',
        objectives: [
          'Build a numerical and a categorical preprocessing pipeline',
          'Combine them with a ColumnTransformer',
          'State the role of SimpleImputer, StandardScaler, and OneHotEncoder',
        ],
        concepts: [
          {
            title: 'Defining the pipelines',
            paragraphs: [
              'Preprocessing steps for numerical and categorical data. numerical_cols = X_train.select_dtypes(include=["int64", "float64"]).columns. categorical_cols = X_train.select_dtypes(include=["object", "category"]).columns.',
        ...
        glossary: [
          { term: 'ColumnTransformer', definition: 'A scikit-learn transformer that applies different preprocessing pipelines to different columns.' },
          { term: 'SimpleImputer', definition: 'A transformer that fills missing values with a statistic — mean for numeric, most_frequent for categorical.' },
        ],
        realWorldExample: {
          scenario: 'Production ML pipeline',
          application: 'Every production model has a ColumnTransformer in front of it — otherwise the same fit / predict code fails on the first missing value or unknown category.',
          impact: 'A ColumnTransformer is what makes a model robust to the messy data that arrives in production.',
        },
        playground: {
          type: 'hp-preprocess',
          title: 'Numerical vs categorical pipelines',
          instructions: 'Switch between the two cards to see the steps for numerics and for categoricals. Notice the different imputation strategies.',
          expectedOutcome: 'Be able to name the two pipelines and the role of each step.',
        },
        quiz: [
          {
            id: 'hpp-q1',
            question: 'What strategy does SimpleImputer use for numeric columns in the book?',
            options: ['mean', 'median', 'most_frequent', 'zero'],
            answerIndex: 0,
            explanation: 'strategy="mean" — the book uses the mean for numeric imputation.',
          },
          {
            id: 'hpp-q2',
            question: 'Why handle_unknown="ignore" in OneHotEncoder?',
            options: [
              'To skip rows with missing values.',
              'To silently pass through categories that did not appear in training (instead of raising an error).',
              'To drop the unknown category automatically.',
              'To randomly guess the encoding.',
            ],
            answerIndex: 1,
            explanation: 'handle_unknown="ignore" makes the encoder return a zero row for unseen categories — important for production data that has new categories.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, click "Numerical pipeline" and "Categorical pipeline" — what is the second step in each?',
          hint: 'After imputation comes scaling (for numerics) or encoding (for categoricals).',
          expectedAnswer: 'Numerical: StandardScaler. Categorical: OneHotEncoder.',
        },
        keyTakeaways: [
          'Two pipelines: numerics (impute mean → scale), categoricals (impute most_frequent → one-hot encode).',
          'ColumnTransformer stitches the two together per column type.',
          'handle_unknown="ignore" makes the model robust to new categories at predict time.',
        ],
      },
      {
        id: 'hp-rf',
        number: 119,
        title: '119. House Price Prediction — Random Forest Model',
        subtitle: 'Fit a Random Forest Regressor in a Pipeline, predict the test set, and report MSE — the book\'s better baseline.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '5 mins',
        difficulty: 'Intermediate',
        objectives: [
          'Define a Random Forest model inside a Pipeline with the preprocessor',
          'Fit, predict, and compute the test MSE',
          'Compare the MSE to the SVM baseline',
        ],
        concepts: [
          {
            title: 'Random Forest in a Pipeline',
            paragraphs: [
              'As our data is ready. We will start our machine learning process. We will use Random forest and SVM.',
              '# Define the Random Forest model within a pipeline that includes preprocessing. rf_model = Pipeline(steps=[ ("preprocessor", preprocessor), ("regressor", RandomForestRegressor(n_estimators=100, random_state=42)) ]).',
              '# Fit the Random Forest model. rf_model.fit(X_train, y_train).',
              '# Predict and evaluate the Random Forest model. y_pred_rf = rf_model.predict(X_test). mse_rf = mean_squared_error(y_test, y_pred_rf). print("Random Forest MSE:", mse_rf).',
              'Random Forest MSE: 857,641,621.2334174.',
            ],
            analogy: {
              concept: 'Pipeline',
              metaphor: 'A factory assembly line — the raw wood comes in one side and a finished house comes out the other.',
              why: 'The same fit / predict code handles preprocessing at every call, so you cannot forget to scale at predict time.',
            },
          },
        ],
        glossary: [
          { term: 'n_estimators', definition: 'The number of trees in the Random Forest — 100 in the book.' },
        ],
        realWorldExample: {
          scenario: 'Tuning the Random Forest to push MSE lower',
          application: 'Try n_estimators=500, add max_features="sqrt", set min_samples_leaf=5 — the book invites you to "do some more EDA and try to get a better MSE value."',
          impact: 'Tuning a single hyperparameter is often the difference between a shipped model and a benchwarmer.',
        },
        playground: {
          type: 'hp-rf',
          title: 'Random Forest MSE vs SVR MSE',
          instructions: 'Look at the bar chart — RF MSE is ~9× smaller than SVR MSE. The book uses RF as the better baseline.',
          expectedOutcome: 'Be able to compare the two book MSEs and explain why RF wins on this dataset.',
        },
        quiz: [
          {
            id: 'hprf-q1',
            question: 'What MSE did the book report for the Random Forest?',
            options: ['85,764,162', '857,641,621', '8,576,416,210', '857,641,621,233'],
            answerIndex: 1,
            explanation: 'The book reports Random Forest MSE: 857,641,621.2334174.',
          },
          {
            id: 'hprf-q2',
            question: 'Why is the Random Forest MSE much smaller than the SVM MSE?',
            options: [
              'The Random Forest is overfitting.',
              'A linear SVR cannot capture the non-linearities in 80 features; the RF can.',
              'The book used different test sets for the two.',
              'SVMs always do worse than tree ensembles.',
            ],
            answerIndex: 1,
            explanation: 'A linear SVR is restricted to a linear decision function; the RF can model non-linear interactions in 80 features.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, by what factor is the SVR MSE larger than the RF MSE?',
          hint: '7,854,595,775 / 857,641,621 ≈ ?',
          expectedAnswer: 'About 9.2× — SVR is roughly 9× worse than RF on this dataset.',
        },
        keyTakeaways: [
          'Pipeline = preprocessor + regressor in a single fit / predict object.',
          'Random Forest (n_estimators=100) achieves MSE 857,641,621 on the test set.',
          'The book invites you to tune the model — try n_estimators=500, min_samples_leaf, max_features.',
        ],
      },
      {
        id: 'hp-svm',
        number: 120,
        title: '120. House Price Prediction — SVM Model',
        subtitle: 'Fit a default SVR inside the same pipeline — a much weaker baseline than the Random Forest.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '4 mins',
        difficulty: 'Intermediate',
        objectives: [
          'Define an SVR model inside a Pipeline with the same preprocessor',
          'Fit, predict, and compute the test MSE',
          'Compare to the Random Forest baseline',
        ],
        concepts: [
          {
            title: 'SVM in a Pipeline',
            paragraphs: [
              '# Define the SVM model within the same pipeline structure. svm_model = Pipeline(steps=[ ("preprocessor", preprocessor), ("regressor", SVR()) ]).',
              '# Fit the SVM model. svm_model.fit(X_train, y_train). # Make predictions and evaluate the model. y_pred = model.predict(X_test). mse_RF = mean_squared_error(y_test, y_pred). print("Mean Squared Error:", mse_RF).',
              '# Predict and evaluate the SVM model. y_pred_svm = svm_model.predict(X_test). mse_svm = mean_squared_error(y_test, y_pred_svm). print("SVM MSE:", mse_svm).',
              'SVM MSE: 7,854,595,775.061981.',
            ],
          },
        ],
        glossary: [
          { term: 'SVR', definition: 'Support Vector Regression — SVM extended to predict continuous targets.' },
        ],
        realWorldExample: {
          scenario: 'Using SVR effectively',
          application: 'A linear SVR with default C=1 and no scaling is the worst-case setting. With feature scaling, kernel="rbf", and tuned C and ε, SVR is competitive on small tabular datasets.',
          impact: 'The book\'s SVR baseline is a starting point — the message is that linear SVR is too restrictive for this dataset.',
        },
        playground: {
          type: 'hp-svm',
          title: 'Linear SVR on a 2-D toy example',
          instructions: 'Drag the Slope and Intercept sliders. The line tracks the true signal; the SVR fits a single line through the noise.',
          expectedOutcome: 'See that the linear SVR cannot follow curvature — a fundamental limit when the underlying relationship is non-linear.',
        },
        quiz: [
          {
            id: 'hps-q1',
            question: 'What MSE did the book report for the SVR?',
            options: ['85,764,162', '857,641,621', '7,854,595,775', '78,545,957,750'],
            answerIndex: 2,
            explanation: 'SVM MSE: 7,854,595,775.061981 — ~9× worse than RF.',
          },
          {
            id: 'hps-q2',
            question: 'Why does the book\'s SVR do so much worse than RF?',
            options: [
              'The book used a bad kernel.',
              'SVMs always fail on tabular data.',
              'A default linear SVR cannot capture non-linearities in 80 features; RF can model interactions.',
              'The book accidentally swapped the labels.',
            ],
            answerIndex: 2,
            explanation: 'Default SVR uses a linear kernel — that is the wrong tool for a dataset with many non-linear feature interactions.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, set the slope to 1 and the intercept to 5. Does the linear SVR fit the noisy points well?',
          hint: 'Look at the chart — the line passes near the centre of the data but misses the upward trend.',
          expectedAnswer: 'No — the line is constrained to be straight, so it underfits the curvature in the data.',
        },
        keyTakeaways: [
          'SVM in a Pipeline: same preprocessor, regressor=SVR().',
          'Test MSE 7,854,595,775 — ~9× worse than RF.',
          'Try kernel="rbf" with tuned C and ε to make SVR competitive.',
        ],
      },
      {
        id: 'covid-overview',
        number: 121,
        title: '121. COVID-19 — Project Overview & Workflow',
        subtitle: 'A time-series prediction project on the OWID COVID dataset — LSTM and SVR side by side.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '4 mins',
        difficulty: 'Beginner',
        objectives: [
          'State the dataset (OWID COVID-19, country-level, daily)',
          'Outline the seven workflow steps',
          'Name the two models (LSTM and linear SVR) the book compares',
        ],
        concepts: [
          {
            title: 'The project',
          ...
          {
            title: 'The seven workflow steps',
            paragraphs: [
              '1. Setup: import everything (pandas, numpy, matplotlib, plotly, scipy, sklearn, tensorflow, ipywidgets).',
              '2. Load: df = pd.read_csv("https://covid.ourworldindata.org/data/owid-covid-data.csv").',
              '3. EDA: pick one country, group by location, sort by date.',
              '4. Preprocess: fill NaN with 0, parse date, interpolate vaccination columns, drop pre-vaccination rows.',
              '5. LSTM: sequence model for new_cases vs people_fully_vaccinated.',
              '6. SVR: linear-kernel regression baseline.',
              '7. Compare: MAE, MSE, RMSE.',
            ],
          },
        ],
        glossary: [
          { term: 'LSTM', definition: 'Long Short-Term Memory — a recurrent neural network that can learn long-range dependencies in sequences.' },
        ],
        realWorldExample: {
          scenario: 'Modelling an infectious-disease curve',
          application: 'LSTM on the OWID daily series captures the temporal pattern of new_cases; SVR with a linear kernel is a flat baseline.',
          impact: 'The book\'s LSTM is one of many ways to forecast epidemic curves — much better than naive linear extrapolation.',
        },
        playground: {
          type: 'covid-overview',
          title: 'COVID-19 workflow',
          instructions: 'Read the seven steps in the chart. The four cards below highlight the most important ones.',
          expectedOutcome: 'Be able to recite the seven steps and name the two models.',
        },
        quiz: [
          {
            id: 'covo-q1',
            question: 'Which dataset does the book load for the COVID project?',
            options: ['Johns Hopkins CSSE', 'OWID COVID dataset', 'WHO dataset', 'ECDC dataset'],
            answerIndex: 1,
            explanation: 'The book: pd.read_csv("https://covid.ourworldindata.org/data/owid-covid-data.csv") — OWID.',
          },
          {
            id: 'covo-q2',
            question: 'Which two models does the book train on the COVID sequences?',
            options: ['LSTM and SVR', 'ARIMA and Prophet', 'Random Forest and XGBoost', 'Linear and Polynomial Regression'],
            answerIndex: 0,
            explanation: 'The book: "we will use a lot of machine learning models, including LSTM, a time series prediction" + SVR.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, list the four steps shown in the card row.',
          hint: 'Read the four cards at the bottom of the panel.',
          expectedAnswer: 'Setup, Load, EDA, Preprocess — the first half of the workflow.',
        },
        keyTakeaways: [
          'COVID-19 project: time-series prediction on the OWID daily dataset.',
          'Two models: LSTM (sequence model) and linear-kernel SVR (baseline).',
          'Seven-step workflow: setup, load, EDA, preprocess, LSTM, SVR, compare.',
        ],
      },
      {
        id: 'covid-load',
        number: 122,
        title: '122. COVID-19 — Data Loading & Schema',
        subtitle: 'Pull the OWID CSV and inspect its columns — pick a country and the right features for the rest of the project.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '4 mins',
        difficulty: 'Beginner',
        objectives: [
          'Load a remote CSV with pandas',
          'Inspect the columns and select a relevant subset',
          'Pick one country for the EDA',
        ],
        concepts: [
          {
            title: 'Loading and selecting',
            paragraphs: [
              'Import the data. df = pd.read_csv("https://covid.ourworldindata.org/data/owid-covid-data.csv"). df. df.columns.',
              'Now your task is to take one country and perform EDA of that particular country. In this step we will make a new dataframe using the columns we need. selected_columns = [ "iso_code", "continent", "location", "date", "new_cases", "new_deaths", "hosp_patients", "total_vaccinations", "people_vaccinated", "people_fully_vaccinated", ]. data = df[selected_columns]. data.',
            ],
          },
        ],
        glossary: [
          { term: 'OWID', definition: 'Our World in Data — an open dataset of country-level COVID-19 statistics maintained at ourworldindata.org.' },
        ],
        realWorldExample: {
          scenario: 'A time-series analyst at a public-health agency',
          application: 'The same OWID CSV is the standard source for time-series COVID analyses — it is updated daily and is the de facto reference dataset.',
          impact: 'Reproducibility: a single CSV is enough to rerun any country-level analysis.',
        },
        playground: {
          type: 'covid-load',
          title: 'Inspect the OWID schema',
          instructions: 'Click each column on the left. The right panel shows what the column contains.',
          expectedOutcome: 'Be able to name the 10 selected columns and what each one is.',
        },
        quiz: [
          {
            id: 'cvl-q1',
            question: 'Which of these columns is NOT in the book\'s selected_columns list?',
            options: ['new_cases', 'people_fully_vaccinated', 'hosp_patients', 'gdp_per_capita'],
            answerIndex: 3,
            explanation: 'The book\'s list has 10 columns — gdp_per_capita is not in it.',
          },
          {
            id: 'cvl-q2',
            question: 'Why does the book say "take one country"?',
            options: [
              'To reduce compute time.',
              'The patterns differ by country; you study one thoroughly before generalising.',
              'There is only one country in the dataset.',
              'The model only supports one country at a time.',
            ],
            answerIndex: 1,
            explanation: 'The book wants you to focus on one country and explore its patterns in detail.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, click "date". What is its type?',
          hint: 'The card shows the type in the right panel.',
          expectedAnswer: 'datetime',
        },
        keyTakeaways: [
          'OWID CSV: country-level, daily granularity.',
          'selected_columns: iso_code, continent, location, date, new_cases, new_deaths, hosp_patients, total_vaccinations, people_vaccinated, people_fully_vaccinated.',
          'Pick one country for EDA — patterns differ by country.',
        ],
      },
      {
        id: 'covid-prep',
        number: 123,
        title: '123. COVID-19 — Preprocessing & Interpolation',
        subtitle: 'Fill NaN, parse dates, sort by location + date, interpolate missing vaccination values, and drop pre-vaccination rows.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '6 mins',
        difficulty: 'Intermediate',
        objectives: [
          'Fill NaN values in the numeric columns with 0',
          'Parse the date column and sort by location + date',
          'Apply linear interpolation to fill the early vaccination zeros',
          'Drop rows where people_fully_vaccinated is 0',
        ],
        concepts: [
          {
            title: 'Preprocessing the data',
            paragraphs: [
              'numeric_columns = ["new_cases", "new_deaths", "hosp_patients", "total_vaccinations", "people_vaccinated", "people_fully_vaccinated"]. data[numeric_columns] = data[numeric_columns].fillna(0).',
              'Convert the \'date\' column to datetime. data["date"] = pd.to_datetime(data["date"]).',
              'Now we group the data by the location. agg_data = data.groupby("location").apply(lambda group: group.sort_values("date")).reset_index(drop=True). agg_data["daily_vaccination_rate"] = agg_data.groupby("location")["people_vaccinated"].diff() / agg_data["new_cases"].',
              'from scipy.interpolate import interp1d. # Find the indices with \'0.0\' values in the columns. zero_indices = (data["total_vaccinations"] == 0) & (data["people_vaccinated"] == 0) & (data["people_fully_vaccinated"] == 0).',
              'Create an interpolation function using linear interpolation. linear_interp = interp1d(date_values[~zero_indices], vaccination_values[~zero_indices], axis=0, kind="linear", fill_value="extrapolate").',
              'Interpolate missing values for the \'0.0\' values. interpolated_values = linear_interp(date_values[zero_indices]). data.loc[zero_indices, ["total_vaccinations", "people_vaccinated", "people_fully_vaccinated"]] = interpolated_values. data = data.drop(["date"], axis=1).',
              'we will remove all the rows where people_full_vaccinated is zero. Because we want to see the infection rate after the vaccination starts. data = data[data["people_fully_vaccinated"] != 0]. data.shape (246252, 9).',
            ],
          },
        ],
        glossary: [
          { term: 'Linear interpolation', definition: 'Fitting a straight line between two known points and using it to fill the missing values in between.' },
        ],
        realWorldExample: {
          scenario: 'Cleaning a public-health time series',
          application: 'Early in the pandemic, vaccination counts are zero by definition — interpolating over the zero block fills the early ramp-up without distorting the rest of the series.',
          impact: 'The dataset shrinks from a few hundred thousand rows to (246,252, 9) — ready for sequence modelling.',
        },
        playground: {
          type: 'covid-prep',
          title: 'Seven preprocessing steps',
          instructions: 'Read the seven cards. Notice the order: fill → parse → sort → compute rate → interpolate → drop date → drop zero rows.',
          expectedOutcome: 'Be able to list the seven steps in order and explain why each one is needed.',
        },
        quiz: [
          {
            id: 'cvp-q1',
            question: 'Why does the book fill NaN with 0?',
            options: [
              'Missing values mean the count was not reported — the book treats them as 0 for simplicity.',
              'NaN is invalid in scikit-learn.',
              'It is the only way to interpolate.',
              'It is a typo.',
            ],
            answerIndex: 0,
            explanation: 'Missing counts are treated as 0 — the book does not differentiate "no data" from "no cases" here.',
          },
          {
            id: 'cvp-q2',
            question: 'Why drop rows where people_fully_vaccinated is 0?',
            options: [
              'To make the dataset smaller.',
              'Because the book wants to study the post-vaccination era — when people_fully_vaccinated is non-zero.',
              'To avoid TypeError.',
              'To balance the classes.',
            ],
            answerIndex: 1,
            explanation: 'The book: "we will remove all the rows where people_full_vaccinated is zero. Because we want to see the infection rate after the vaccination starts."',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, identify the step that uses scipy.interpolate.interp1d.',
          hint: 'It is the one that fills the early vaccination zeros.',
          expectedAnswer: 'Linear interpolation.',
        },
        keyTakeaways: [
          'fillna(0) on numeric columns; pd.to_datetime on date; sort by location + date.',
          'scipy.interpolate.interp1d fills the early vaccination zeros with linear interpolation.',
          'Drop rows with people_fully_vaccinated == 0 to keep only the post-vaccination era.',
        ],
      },
      {
        id: 'covid-eda',
        number: 124,
        title: '124. COVID-19 — Exploratory Analysis (Per Country)',
        subtitle: 'Pick a country, plot new_cases, new_deaths and the vaccination curve, and prepare to predict new_cases from people_fully_vaccinated.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '5 mins',
        difficulty: 'Beginner',
        objectives: [
          'Pick one country and look at its daily series',
          'Plot new_cases, new_deaths, and cumulative vaccinations',
          'Identify the period of post-vaccination data to model on',
        ],
        concepts: [
          {
            title: 'Per-country EDA',
            paragraphs: [
              'Now we will dive into the machine learning part.',
            ],
            analogy: {
              concept: 'Pick a country',
              metaphor: 'Zoom in on a single neighbourhood before you try to model the whole city.',
              why: 'Country-level patterns (waves, vaccination rollouts) are clearer than global aggregates.',
            },
          },
        ],
        glossary: [
          { term: 'Wave', definition: 'A sustained rise and fall in new_cases — typical of an epidemic curve.' },
        ],
        realWorldExample: {
          scenario: 'A public-health analyst choosing a country to model',
          application: 'Picking a country with a clean vaccination rollout (no missing data) gives the cleanest training signal for the LSTM.',
          impact: 'Good EDA = a smoother modelling target = a more accurate forecast.',
        },
        playground: {
          type: 'covid-eda',
          title: 'Daily cases, deaths and vaccinations',
          instructions: 'Click each country on the left. The right chart shows new_cases, new_deaths, and cumulative vaccinations for that country.',
          expectedOutcome: 'See the wave shape and the lag between cases and vaccinations in your chosen country.',
        },
        quiz: [
          {
            id: 'cve-q1',
            question: 'Why does the book suggest picking one country for EDA?',
            options: [
              'The CSV only has one country.',
              'Country-level patterns differ; a single country lets you see the wave shape clearly.',
              'It is faster.',
              'LSTM only works on one country.',
            ],
            answerIndex: 1,
            explanation: 'The book: "your task is to take one country and perform EDA of that particular country."',
          },
          {
            id: 'cve-q2',
            question: 'On a daily chart, what does a "wave" look like?',
            options: [
              'A random walk.',
              'A sustained rise and fall in new_cases.',
              'A monotonic increase.',
              'A horizontal line at zero.',
            ],
            answerIndex: 1,
            explanation: 'An epidemic wave is a sustained rise and fall in the case count.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, switch to the third country. Does the wave peak earlier or later than the first?',
          hint: 'Each country\'s wave is centred at a different day.',
          expectedAnswer: 'It depends on the country — the simulator draws each wave with a different peak day.',
        },
        keyTakeaways: [
          'Pick one country for per-country EDA — patterns differ.',
          'Plot new_cases, new_deaths, and cumulative vaccinations on the same chart.',
          'The post-vaccination period is the modelling target.',
        ],
      },
      {
        id: 'covid-lstm',
        number: 125,
        title: '125. COVID-19 — LSTM Time Series Model',
        subtitle: 'Build a sequence model on people_fully_vaccinated → new_cases with sequence_length=10 and report MAE, MSE, RMSE.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '7 mins',
        difficulty: 'Advanced',
        objectives: [
          'Build a 50-unit LSTM with a Dense(1) head',
          'Train on 10-step sequences for 10 epochs',
          'Compute MAE, MSE, RMSE on the held-out test set',
        ],
        concepts: [
          {
            title: 'LSTM',
            paragraphs: [
              'LSTM. from sklearn.model_selection import train_test_split. from sklearn.preprocessing import MinMaxScaler. from tensorflow.keras.models import Sequential. from tensorflow.keras.layers import LSTM, Dense.',
              'Normalize the data. scaler = MinMaxScaler(). data[["new_cases", "people_fully_vaccinated"]] = scaler.fit_transform(data[["new_cases", "people_fully_vaccinated"]]).',
              'Split data into features (X) and target (y). X = data["people_fully_vaccinated"].values. y = data["new_cases"].values.',
              ...
              'Create sequences. sequence_length = 10. You can adjust this hyperparameter. X_train_seq, y_train_seq = [], []. for i in range(len(X_train) - sequence_length): X_train_seq.append(X_train[i:i+sequence_length]). y_train_seq.append(y_train[i+sequence_length]). X_train_seq, y_train_seq = np.array(X_train_seq), np.array(y_train_seq).',
              'Build the LSTM model. model = Sequential(). model.add(LSTM(50, activation="relu", input_shape=(sequence_length, 1))). model.add(Dense(1)). model.compile(optimizer="adam", loss="mse").',
              'Train the model. model.fit(X_train_seq, y_train_seq, epochs=10, batch_size=32).',
              'Evaluate the model. from sklearn.metrics import mean_absolute_error, mean_squared_error.',
              'Mean Absolute Error (MAE): 0.00251675. Mean Squared Error (MSE): 0.00025270. Root Mean Squared Error (RMSE): 0.01589647.',
            ],
          },
        ],
        glossary: [
          { term: 'MinMaxScaler', definition: 'A scikit-learn transformer that scales features to a [0, 1] range — required for stable LSTM training.' },
          { term: 'sequence_length', definition: 'The number of past timesteps the LSTM sees to predict the next one — 10 in the book.' },
        ],
        realWorldExample: {
          scenario: 'Forecasting new_cases from people_fully_vaccinated',
          application: 'The LSTM is essentially asking: "given the last 10 days of vaccinations, what is today\'s new_cases?" — a short-horizon sequence-to-one regression.',
          impact: 'With 10 epochs the model already reaches a small MAE on the scaled data — the book invites you to "implement other machine learning models" to compare.',
        },
        playground: {
          type: 'covid-lstm',
          title: 'LSTM training curves',
          instructions: 'Drag the Epochs and LR-multiplier sliders. The chart shows train / val loss over time.',
          expectedOutcome: 'See the loss decrease with more epochs and watch the validation loss start to diverge if you push too hard.',
        },
        quiz: [
          {
            id: 'cvl-q1',
            question: 'What sequence_length does the book use?',
            options: ['1', '5', '10', '30'],
            answerIndex: 2,
            explanation: 'The book: sequence_length = 10. You can adjust this hyperparameter.',
          },
          {
            id: 'cvl-q2',
            question: 'What MSE did the LSTM achieve?',
            options: ['0.00025', '0.025', '0.25', '2.5'],
            answerIndex: 0,
            explanation: 'The book reports MSE: 0.00025270.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, set epochs to 1. Does the loss decrease?',
          hint: 'A single epoch is rarely enough to converge an LSTM.',
          expectedAnswer: 'The loss drops a little but the model is still far from converged — the book trains for 10 epochs.',
        },
        keyTakeaways: [
          'LSTM(50, activation="relu", input_shape=(sequence_length, 1)) → Dense(1), compile(adam, mse).',
          'Trained for 10 epochs, batch size 32 on 80% / 20% train / test split.',
          'MAE: 0.0025, MSE: 0.00025, RMSE: 0.0159.',
        ],
      },
      {
        id: 'covid-svr',
        number: 126,
        title: '126. COVID-19 — SVR Baseline Model',
        subtitle: 'A linear-kernel SVR on the same sequences — a flat baseline against which to compare the LSTM.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '4 mins',
        difficulty: 'Intermediate',
        objectives: [
          'Fit a linear-kernel SVR on the same 10-step sequences',
          'Report MAE, MSE, RMSE',
          'Compare to the LSTM and pick the better model',
        ],
        concepts: [
          {
            title: 'SVR',
            paragraphs: [
              'SVR. from sklearn.svm import SVR. from sklearn.metrics import mean_squared_error.',
              'Create a Support Vector Regressor. svr = SVR(kernel="linear").',
              'Train the model. svr.fit(X_train_seq, y_train_seq).',
              'Make predictions using the SVR model. y_pred_svr = svr.predict(X_test_seq).',
              'Calculate the Mean Squared Error (MSE). mse_svr = mean_squared_error(y_test[sequence_length:], y_pred_svr).',
              'Calculate the Mean Absolute Error (MAE). mae = mean_absolute_error(y_test[sequence_length:], y_pred_svr). mse = mean_squared_error(y_test[sequence_length:], y_pred_svr). rmse = np.sqrt(mse).',
              'Now your job is to implement other machine learning models. For example gredient boosting, random forest etc.',
            ],
          },
        ],
        glossary: [
          { term: 'Linear-kernel SVR', definition: 'A Support Vector Regressor with a linear kernel — equivalent to ridge regression but with an ε-insensitive loss.' },
        ],
        realWorldExample: {
          scenario: 'Choosing between LSTM and SVR on a time series',
          application: 'LSTM is a sequence model; SVR is not. When the signal has any temporal structure, the LSTM wins — and it does, by ~2× MAE in the book.',
          impact: 'Use the simpler model only when the gap is small — the book\'s LSTM is the clear choice on the COVID sequences.',
        },
        playground: {
          type: 'covid-svr',
          title: 'LSTM vs SVR on the COVID sequences',
          instructions: 'Compare the bar chart — LSTM has roughly half the MAE of SVR. The book invites you to add gradient boosting, random forest, etc.',
          expectedOutcome: 'See that the sequence model wins on a sequence problem; the linear SVR cannot capture the temporal structure.',
        },
        quiz: [
          {
            id: 'cvs-q1',
            question: 'What kernel does the book\'s SVR use?',
            options: ['linear', 'rbf', 'polynomial', 'sigmoid'],
            answerIndex: 0,
            explanation: 'The book: svr = SVR(kernel="linear").',
          },
          {
            id: 'cvs-q2',
            question: 'Why does the LSTM beat the SVR here?',
            options: [
              'LSTMs always win.',
              'The signal has temporal structure; the LSTM is a sequence model, the linear SVR is not.',
              'The SVR is buggy.',
              'The book used a bad loss function.',
            ],
            answerIndex: 1,
            explanation: 'Sequence data favours a sequence model — LSTMs have memory of the last 10 steps, SVR does not.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, by what factor is the SVR MAE higher than the LSTM MAE?',
          hint: 'SVR MAE ≈ 0.0045, LSTM MAE ≈ 0.0025.',
          expectedAnswer: 'About 1.8× — the linear SVR is roughly twice as bad on the test set.',
        },
        keyTakeaways: [
          'SVR(kernel="linear") is a flat baseline.',
          'LSTM cuts MAE roughly in half thanks to the temporal structure of the data.',
          'The book\'s challenge: add gradient boosting, random forest, etc., and compare.',
        ],
      },
      {
        id: 'pneu-overview',
        number: 127,
        title: '127. Pneumonia Diagnosis — Project Overview',
        subtitle: 'A binary image classification project on chest X-rays — 5,216 train images, 2 classes (NORMAL, PNEUMONIA), 256×256 input.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '4 mins',
        difficulty: 'Beginner',
        objectives: [
          'State the task: binary classification of chest X-ray images',
          'Name the dataset split (5,216 / 16 / 624)',
          'Identify the image size, batch size, epochs, and number of classes',
        ],
        concepts: [
          {
            title: 'The project',
            paragraphs: [
              'In this project we will use images of chest X-ray and predict whether the patient has pneumonia or not. But before that, let\'s learn a little about CNN.',
          ...
          },
          {
            title: 'Setup',
            paragraphs: [
              'img_width, img_height = 256, 256. batchsize = 32. epochs = 10. num_of_class = 2.',
              'train = keras.utils.image_dataset_from_directory( directory="/kaggle/input/chest-xray-pneumonia/chest_xray/train", labels="inferred", label_mode="categorical", batch_size=batchsize, image_size=(img_width, img_height) ).',
              'validation = keras.utils.image_dataset_from_directory( directory="/kaggle/input/chest-xray-pneumonia/chest_xray/val", labels="inferred", label_mode="categorical", batch_size=batchsize, image_size=(img_width, img_height) ).',
              'test = keras.utils.image_dataset_from_directory( directory="/kaggle/input/chest-xray-pneumonia/chest_xray/test", labels="inferred", label_mode="categorical", batch_size=batchsize, image_size=(img_width, img_height) ).',
              'Found 5216 files belonging to 2 classes. Found 16 files belonging to 2 classes. Found 624 files belonging to 2 classes.',
            ],
          },
        ],
        glossary: [
          { term: 'Transfer learning', definition: 'Reusing a model pre-trained on a large dataset (e.g. ImageNet) and fine-tuning it on a smaller, task-specific dataset.' },
        ],
        realWorldExample: {
          scenario: 'A hospital radiology department triages chest X-rays',
          application: 'A CNN classifies new X-rays as NORMAL or PNEUMONIA in seconds, flagging urgent cases for the radiologist — a textbook use of CNN-based transfer learning.',
          impact: 'Faster triage means faster treatment — and lives saved.',
        },
        playground: {
          type: 'pneu-overview',
          title: 'Dataset split & image parameters',
          instructions: 'Read the chart on the left (the dataset split) and the panel on the right (image size, batch size, epochs, classes).',
          expectedOutcome: 'Be able to recite the train / val / test split, the image size, batch size, epochs and number of classes.',
        },
        quiz: [
          {
            id: 'pn1-q1',
            question: 'How many training images are in the chest X-ray dataset?',
            options: ['16', '624', '5,216', '50,000'],
            answerIndex: 2,
            explanation: 'The book: Found 5216 files belonging to 2 classes.',
          },
          {
            id: 'pn1-q2',
            question: 'What image size does the book use?',
            options: ['32×32', '128×128', '256×256', '512×512'],
            answerIndex: 2,
            explanation: 'img_width, img_height = 256, 256.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, what is the ratio of training to test images?',
          hint: 'Train = 5,216, Test = 624.',
          expectedAnswer: 'About 8.4 : 1 — typical for medical imaging datasets with a small labelled cohort.',
        },
        keyTakeaways: [
          'Pneumonia diagnosis: binary image classification (NORMAL vs PNEUMONIA).',
          'Dataset: 5,216 train / 16 val / 624 test chest X-rays, 256×256 pixels.',
          'Batch size 32, 10 epochs, 2 classes.',
        ],
      },
      {
        id: 'pneu-prep',
        number: 128,
        title: '128. Pneumonia Diagnosis — Data Loading & Preprocessing',
        subtitle: 'Use image_dataset_from_directory to load the X-rays, convert batches to numpy, stack into arrays, and normalise pixel values.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '5 mins',
        difficulty: 'Intermediate',
        objectives: [
          'Load image datasets with image_dataset_from_directory',
          'Convert tf.data batches to numpy arrays and concatenate them',
          'Normalise pixel values to [0, 1] by dividing by 256',
        ],
        concepts: [
          {
            title: 'Preprocessing the data',
            paragraphs: [
              'Preprocessing the data: # Extracting Features and Labels. x_train = []. y_train = []. x_val = []. y_val = []. x_test = []. y_test = [].',
              'for feature, label in train: x_train.append(feature.numpy()). y_train.append(label.numpy()). for feature, label in test: x_test.append(feature.numpy()). y_test.append(label.numpy()). for feature, label in validation: x_val.append(feature.numpy()). y_val.append(label.numpy()).',
              'Concatenate the lists to get the full \'x\' and \'y\' arrays. x_train = np.concatenate(x_train, axis=0). x_val = np.concatenate(x_val, axis=0). x_test = np.concatenate(x_test, axis=0). y_train = np.concatenate(y_train, axis=0). y_val = np.concatenate(y_val, axis=0). y_test = np.concatenate(y_test, axis=0).',
              'Pixel Value Scaling for Datasets: Normalizing and Standardizing the Data. x_train=x_train/256. x_val=x_val/256. x_test=x_test/256.',
            ],
          },
        ],
        glossary: [
          { term: 'image_dataset_from_directory', definition: 'A Keras utility that loads images from disk into a tf.data.Dataset, automatically inferring labels from folder names.' },
          { term: 'tf.data.Dataset', definition: 'A TensorFlow pipeline object that yields batches of tensors — efficient for training on large datasets.' },
        ],
        realWorldExample: {
          scenario: 'A medical-imaging startup\'s ETL pipeline',
          application: 'The same three steps — tf.data pipeline, numpy stacking, pixel scaling — is the standard pre-training step for every image classifier.',
          impact: 'Skipping the pixel scaling (leaving pixels in [0, 255]) makes training unstable for most CNNs.',
        },
        playground: {
          type: 'pneu-prep',
          title: 'Four preprocessing steps',
          instructions: 'Read the four cards. The first loads the dataset, the second stacks numpy arrays, the third normalises pixel values, the fourth feeds the model.',
          expectedOutcome: 'Be able to recite the four steps and the role of each one.',
        },
        quiz: [
          {
            id: 'pn2-q1',
            question: 'Why divide by 256?',
            options: [
              'To make the numbers smaller — to [0, 1].',
              'To remove the green channel.',
              'To save memory.',
              'It is a typo.',
            ],
            answerIndex: 0,
            explanation: 'Pixel values are 0-255; dividing by 256 (or 255) scales to [0, 1] for stable training.',
          },
          {
            id: 'pn2-q2',
            question: 'Why use image_dataset_from_directory instead of loading manually?',
            options: [
              'It is faster — Keras handles batching, decoding and label inference.',
              'It is the only way to load images.',
              'It does not need a GPU.',
              'It is required by the book.',
            ],
            answerIndex: 0,
            explanation: 'image_dataset_from_directory handles decoding, resizing, batching and label inference in one call.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, identify the step that normalises pixel values.',
          hint: 'The last step in the list.',
          expectedAnswer: 'x_train /= 256 — divide pixel values by 256 to scale to [0, 1].',
        },
        keyTakeaways: [
          'image_dataset_from_directory loads images, infers labels, and yields batches.',
          'Concatenate tf.data batches into numpy arrays of shape (N, 256, 256, 3).',
          'x_train /= 256 — scale pixel values to [0, 1].',
        ],
      },
      {
        id: 'cnn-core',
        number: 129,
        title: '129. CNNs: Core Components',
        subtitle: 'The four building blocks of every CNN — convolutional layers, activation functions, pooling layers, and fully connected layers.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '6 mins',
        difficulty: 'Intermediate',
        objectives: [
          'Name the four core components of a CNN',
          'Describe what each component does',
          'Recognise the order they appear in a typical CNN',
        ],
        concepts: [
          {
            title: 'Core Components of CNNs',
            paragraphs: [
              'Core Components of CNNs:',
              '1. Convolutional Layers: The fundamental building blocks of a CNN. Each convolutional layer applies numerous filters to its input. Each filter detects features such as edges, corners, or textures by performing a convolution operation between the filter and the input, producing a feature map.',
              '2. Activation Functions: Typically, a non-linear activation function such as ReLU (Rectified Linear Unit) is applied to the output of the convolution operation to introduce non-linear properties into the network, helping it learn more complex patterns.',
              ...
              '4. Fully Connected Layers: After several convolutional and pooling layers, the high-level reasoning in the neural network is done via fully connected layers. Neurons in a fully connected layer have connections to all activations in the previous layer, as seen in regular artificial neural networks. Their outputs are computed by a matrix multiplication followed by a bias offset.',
            ],
          },
        ],
        glossary: [
          { term: 'Filter', definition: 'A small matrix (e.g. 3×3) that slides over the input, computing a dot product at each position to detect a specific feature.' },
        ],
        realWorldExample: {
          scenario: 'Inspecting a real CNN',
          application: 'In PyTorch or Keras, a typical CNN is a stack of Conv2D → ReLU → MaxPool2D repeated, followed by Flatten → Dense → Softmax at the end.',
          impact: 'Knowing the four components lets you read any CNN architecture at a glance.',
        },
        playground: {
          type: 'cnn-core',
          title: 'The four core components',
          instructions: 'Click each card to see the description. Look at the bar chart to see the relative weight of each component in a typical CNN.',
          expectedOutcome: 'Be able to name the four components and describe each one in one sentence.',
        },
        quiz: [
          {
            id: 'cc-q1',
            question: 'Which component detects features like edges and corners?',
            options: ['Activation function', 'Convolutional layer', 'Pooling layer', 'Fully connected layer'],
            answerIndex: 1,
            explanation: 'Convolutional layers apply filters that detect edges, corners, and textures.',
          },
          {
            id: 'cc-q2',
            question: 'Which component is used for high-level reasoning at the end of a CNN?',
            options: ['Convolutional layer', 'Pooling layer', 'Fully connected layer', 'Activation function'],
            answerIndex: 2,
            explanation: 'Fully connected layers do the high-level reasoning after the conv + pool stack.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, name the four components in the order they appear in a typical CNN.',
          hint: 'Conv → Activation → Pool → (repeat) → Fully connected.',
          expectedAnswer: 'Convolutional Layer → Activation Function → Pooling Layer → Fully Connected Layer.',
        },
        keyTakeaways: [
          'Convolutional layers detect local features (edges, corners, textures).',
          'Activation functions (ReLU) add non-linearity.',
          'Pooling layers downsample to control overfitting and parameters.',
          'Fully connected layers do high-level reasoning at the end.',
        ],
      },
      {
        id: 'cnn-how',
        number: 130,
        title: '130. CNNs: How They Work',
        subtitle: 'The forward pass — image → conv + ReLU + pool → abstract features → fully connected → softmax probabilities.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '5 mins',
        difficulty: 'Intermediate',
        objectives: [
          'Walk through the forward pass of a CNN',
          'See how spatial size shrinks while channels grow',
          'Recognise the role of softmax in the final classification',
        ],
        concepts: [
          {
            title: 'How CNNs Work',
            paragraphs: [
              'How CNNs Work: An image is input into the network, which then passes through multiple convolutional, non-linear activation, and pooling layers. These layers extract and transform features from the image. After successive layers, the abstract features (like the presence of shapes or objects) are extracted, and the spatial hierarchy of features is learned.',
            ...
            analogy: {
              ...
              metaphor: 'A pipeline — each stage squeezes the image into a smaller, denser representation of "what is in it" until a softmax turns it into a probability.',
              why: 'A 256×256 image is too big to classify directly — CNNs shrink it feature by feature until only the answer remains.',
            },
          },
        ],
        glossary: [
          { term: 'Softmax', definition: 'A final activation that turns a vector of scores into a probability distribution (sums to 1).' },
        ],
        realWorldExample: {
          scenario: 'A VGG-style CNN on ImageNet',
          application: 'Image is shrunk from 224×224×3 → 7×7×512 by repeated conv + pool, then flattened and fed to a 1000-way softmax for the 1000 ImageNet classes.',
          impact: 'The same idea — conv + pool + flatten + softmax — is the blueprint of every modern image classifier.',
        },
        playground: {
          type: 'cnn-how',
          title: 'Spatial resolution through a VGG-style CNN',
          instructions: 'Read the chart. Notice how spatial size shrinks with each pool while channel count grows with each conv.',
          expectedOutcome: 'See the image shrink from 256×256 to a few pixels of "what is in the picture".',
        },
        quiz: [
          {
            id: 'ch-q1',
            question: 'The book says the forward pass does:',
            options: [
              'Image → conv + activation + pool (repeated) → abstract features → fully connected → softmax.',
              'Image → fully connected → softmax directly.',
              'Image → pool → conv → softmax.',
              'Image → activation → conv → pool → fully connected → conv → softmax.',
            ],
            answerIndex: 0,
            explanation: 'The book: "An image is input… passes through multiple convolutional, non-linear activation, and pooling layers… The final output is typically produced using fully connected layers followed by a classification layer (like softmax)."',
          },
          {
            id: 'ch-q2',
            question: 'What does the softmax at the end of a CNN do?',
            options: [
              'Detects edges.',
              'Downsamples the image.',
              'Turns scores into a probability distribution over the classes.',
              'Adds ReLU non-linearity.',
            ],
            answerIndex: 2,
            explanation: 'Softmax turns a vector of logits into a probability distribution — the class with the highest probability is the prediction.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, at which layer does the spatial size drop below 10?',
          hint: 'Look at the chart — the spatial size drops after every max-pool.',
          expectedAnswer: 'After the 4th max-pool layer — at the "MaxPool 2×2 (size 10)" stage.',
        },
        keyTakeaways: [
          'Image → conv + ReLU + pool → abstract features → fully connected → softmax.',
          'Spatial size shrinks with every pool; channel count grows with every conv.',
          'Softmax turns logits into a probability distribution over the classes.',
        ],
      },
      {
        id: 'cnn-advantages',
        number: 131,
        title: '131. CNNs: Advantages',
        subtitle: 'Three reasons CNNs took over computer vision — parameter sharing, local connectivity, and robustness to image translation.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '4 mins',
        difficulty: 'Beginner',
        objectives: [
          'Name the three advantages of CNNs',
          'Explain parameter sharing in plain language',
          'Connect the advantages to the project\'s pneumonia task',
        ],
        concepts: [
          {
            title: 'Advantages of CNNs',
            paragraphs: [
              'Advantages of CNNs: Parameter Sharing: A feature detector (filter) that\'s useful in one part of the image is probably useful across the entire image, reducing the overall memory footprint required for storing parameters and improving the efficiency of the model.',
        ...
        glossary: [
          { term: 'Parameter sharing', definition: 'The same filter weights are applied at every spatial position — a single 3×3 filter has 9 parameters, not 9 × image pixels.' },
        ],
        realWorldExample: {
          scenario: 'Training on a small medical dataset',
          application: 'Parameter sharing is why CNNs can be trained on small datasets like the 5,216-image pneumonia set — the same filter weights generalise across every position in every image.',
          impact: 'You do not need millions of training images to get a useful CNN — transfer learning + parameter sharing makes small-data CV work.',
        },
        playground: {
          type: 'cnn-advantages',
          title: 'Three advantages of CNNs',
          instructions: 'Read the three cards. Notice that all three reduce the model\'s effective parameter count or its sensitivity to position — both make training easier.',
          expectedOutcome: 'Be able to name the three advantages and link each one to a pneumonia-detection use case.',
        },
        quiz: [
          {
            id: 'ca-q1',
            question: 'Parameter sharing means:',
            options: [
              'The model shares weights with another model.',
              'The same filter weights are applied at every spatial position in the image.',
              'All neurons share the same activation.',
              'Layers share inputs.',
            ],
            answerIndex: 1,
            explanation: 'A 3×3 filter slides over the image and uses the SAME 9 weights at every position.',
          },
          {
            id: 'ca-q2',
            question: 'Why is a CNN robust to image translation?',
            options: [
              'It re-trains on every shift.',
              'Pooling layers downsample the feature maps, so small shifts in the input become small shifts in the output that the classifier can still recognise.',
              'It uses random projections.',
              'It does not have any weights.',
            ],
            answerIndex: 1,
            explanation: 'Pooling layers aggregate neighbourhoods — a small shift in the input becomes a small shift in the pooled output, which the classifier tolerates.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, which advantage is most important for detecting pneumonia regardless of where the opacity sits in the X-ray?',
          hint: 'Pooled features are position-tolerant.',
          expectedAnswer: 'Robustness to image translation — the same filter detects the opacity whether it is in the upper left or lower right of the chest.',
        },
        keyTakeaways: [
          'Parameter sharing: one set of filter weights scans the whole image.',
          'Local connectivity: each filter looks at a small neighbourhood.',
          'Robustness to image translation: pooling makes the output position-tolerant.',
        ],
      },
      {
        id: 'pneumodel',
        number: 132,
        title: '132. Pneumonia Diagnosis — Building the CNN Model (VGG16 + head)',
        subtitle: 'Transfer learning with VGG16 — freeze the backbone, unfreeze the last 3 conv layers, add Dropout + Dense + sigmoid for 2-class output.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '6 mins',
        difficulty: 'Advanced',
        objectives: [
          'Build a transfer-learning model on VGG16',
          'Freeze the backbone and unfreeze the last 3 conv layers',
          'Add Dropout(0.3), Dense(128, L2=0.05), Dropout(0.2), Dense(2, sigmoid)',
        ],
        concepts: [
          {
            title: 'Model building',
            paragraphs: [
              'Model Building: # model building. # We use transfer learning with VGG16 as the base model. def CNN_Model():',
              'base_model = VGG16(weights="imagenet", include_top=False, input_shape=(img_width, img_height, 3)).',
              '# Freeze the base model. for layer in base_model.layers: layer.trainable = False. for i in range(3): base_model.layers[-2-i].trainable = True.',
              'CNN = Sequential(). CNN.add(Input(shape=(img_width, img_height, 3))). CNN.add(base_model). CNN.add(Flatten()). CNN.add(Dropout(0.3)). CNN.add(Dense(128, activation="relu", kernel_regularizer=l2(0.05))). CNN.add(Dropout(0.2)). CNN.add(Dense(2, activation="sigmoid")).',
              'return CNN.',
            ],
          },
        ],
        glossary: [
          { term: 'include_top=False', definition: 'Strip VGG16\'s 1000-way softmax head so you can add your own classifier on top.' },
        ],
        realWorldExample: {
          scenario: 'Fine-tuning VGG16 for chest X-rays',
          application: 'VGG16 was pre-trained on ImageNet — its early layers detect edges and textures that are useful for any image, while the unfrozen last 3 conv layers adapt to chest X-ray specific features.',
          impact: 'Transfer learning lets you reach high accuracy with a fraction of the data a from-scratch CNN would need.',
        },
        playground: {
          type: 'pneumodel',
          title: 'Layer-by-layer model summary',
          instructions: 'Read the layers on the left and the compilation panel on the right. Notice the L2 regulariser on the Dense(128) layer.',
          expectedOutcome: 'Be able to list the layers in order and explain why L2 is added to the head.',
        },
        quiz: [
          {
            id: 'pnm-q1',
            question: 'How many trainable top conv layers does the book unfreeze?',
            options: ['0', '1', '3', 'All of them'],
            answerIndex: 2,
            explanation: 'The book: for i in range(3): base_model.layers[-2-i].trainable = True — unfreezes the last 3 conv layers.',
          },
          {
            id: 'pnm-q2',
            question: 'Why use L2 regularisation on the Dense(128) head?',
            options: [
              'It is required by Keras.',
              'It penalises large weights and helps prevent overfitting on a small medical dataset.',
              'It speeds up training.',
              'It is a typo.',
            ],
            answerIndex: 1,
            explanation: 'L2 shrinks the weights, which is regularisation — important when training on only ~5k images.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, what is the activation of the final Dense(2) layer?',
          hint: 'Read the rightmost layer.',
          expectedAnswer: 'sigmoid — the book uses sigmoid for the 2-class output (not softmax).',
        },
        keyTakeaways: [
          'VGG16(weights="imagenet", include_top=False) — strip the head.',
          'Freeze the backbone; unfreeze the last 3 conv layers for fine-tuning.',
          'Head: Flatten → Dropout(0.3) → Dense(128, ReLU, L2) → Dropout(0.2) → Dense(2, sigmoid).',
        ],
      },
      {
        id: 'pneu-train',
        number: 133,
        title: '133. Pneumonia Diagnosis — Training the CNN',
        subtitle: 'Compile with Adam + binary_crossentropy, train for 10 epochs, and watch the training/validation loss and accuracy curves.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '6 mins',
        difficulty: 'Advanced',
        objectives: [
          'Compile the model with Adam, binary_crossentropy, and accuracy',
        ...
        concepts: [
          {
            ...
            paragraphs: [
              '# Training The CNN. model = CNN_Model(). model.compile(optimizer="adam", loss="binary_crossentropy", metrics=["accuracy"]). model.summary(). # Visualized Layers of CNN. keras.utils.plot_model(model, show_shapes=True).',
              'hist = model.fit(x_train, y_train, epochs=epochs, validation_data=(x_val, y_val)). print(hist.history). best_score = max(hist.history["val_accuracy"]). print(f"Best Validation score is: {best_score}").',
              '# evaluate the model. # Extract the training and validation loss values from the history object. train_loss = hist.history["loss"]. val_loss = hist.history["val_loss"]. # Create a list of epoch numbers (1 to number of epochs). epochs = range(1, len(train_loss) + 1). # Plot the loss graph. plt.plot(epochs, train_loss, label="Training Loss"). plt.plot(epochs, val_loss, label="Validation Loss"). plt.title("Training and Validation Loss"). plt.xlabel("Epochs"). plt.ylabel("Loss"). plt.legend(). plt.grid(True). plt.show().',
              '# Extract the training and validation loss values from the history object. train_accuracy = hist.history["accuracy"]. val_accuracy = hist.history["val_accuracy"]. # Create a list of epoch numbers (1 to number of epochs). epochs = range(1, len(train_accuracy) + 1). # Plot the loss graph. plt.plot(epochs, train_accuracy, label="Training Acc"). plt.plot(epochs, val_accuracy, label="Validation Acc"). plt.title("Training and Validation Accuracy"). plt.xlabel("Epochs"). plt.ylabel("Acc"). plt.legend(). plt.grid(True). plt.show().',
            ],
          },
        ],
        glossary: [
          { term: 'binary_crossentropy', definition: 'The standard loss for binary classification — also called log loss.' },
        ],
        realWorldExample: {
          scenario: 'Watching a CNN train',
          application: 'Plotting training and validation loss on the same chart is the standard sanity check — divergence means overfitting; convergence means the model is learning both train and val.',
          impact: 'The two-curve plot is the cheapest debugging tool in deep learning — glance at it before reading any number.',
        },
        playground: {
          type: 'pneu-train',
          title: 'Training & validation loss and accuracy',
          instructions: 'Drag the Epochs and LR-multiplier sliders. The top chart shows loss, the bottom shows accuracy. Watch the validation curve.',
          expectedOutcome: 'See the loss decrease and accuracy rise; watch for the validation curve to diverge from the training curve.',
        },
        quiz: [
          {
            id: 'pnt-q1',
            question: 'The book compiles the model with:',
            options: ['SGD + mse', 'Adam + binary_crossentropy + accuracy', 'RMSprop + hinge', 'Adagrad + categorical_crossentropy'],
            answerIndex: 1,
            explanation: 'model.compile(optimizer="adam", loss="binary_crossentropy", metrics=["accuracy"])',
          },
          {
            id: 'pnt-q2',
            question: 'When training vs validation loss diverge, what is happening?',
            options: ['The model is underfitting.', 'The model is overfitting — it is learning the training set but not generalising.', 'The dataset is too small.', 'The loss is wrong.'],
            answerIndex: 1,
            explanation: 'Diverging train/val loss = overfitting. Add dropout, regularisation, or data; or stop training earlier (early stopping).',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, what is the relationship between training and validation loss in the final epoch?',
          hint: 'Look at the final point in the loss chart.',
          expectedAnswer: 'In the demo, validation loss is slightly above training loss — a small gap, indicating mild overfitting.',
        },
        keyTakeaways: [
          'Compile with Adam, binary_crossentropy, accuracy.',
          'Train for 10 epochs, batch size 32.',
          'Plot train vs val loss and accuracy; divergence = overfitting.',
        ],
      },
      {
        id: 'pneu-eval',
        number: 134,
        title: '134. Pneumonia Diagnosis — Evaluation & Results',
        subtitle: 'Report accuracy, precision, recall, F1, and the ROC curve — and the book\'s challenge: tweak the model and try to do better.',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '5 mins',
        difficulty: 'Advanced',
        objectives: [
          'Read the confusion matrix and the four metrics',
          'Plot the ROC curve for the binary classifier',
          'See why recall matters more than precision in medical diagnosis',
        ],
        concepts: [
          {
            title: 'Evaluation',
            paragraphs: [
              'Now try to add or subtract dense layers. See what happenes. Play with the values of epoch. See what changes happens. Try to make it more efficient.',
            ],
          },
        ],
        glossary: [
          { term: 'Recall (sensitivity)', definition: 'The fraction of true positives the model catches — the most important metric when missing a positive case is expensive.' },
        ],
        realWorldExample: {
          scenario: 'Tuning a medical-imaging CNN',
          application: 'In pneumonia diagnosis, missing a positive case (false negative) is worse than a false alarm (false positive) — so recall is the priority. The book\'s challenge: tweak the head and the epochs to push accuracy and recall up.',
          impact: 'Each percentage point of recall in pneumonia screening is hundreds of early diagnoses.',
        },
        playground: {
          type: 'pneu-eval',
          title: 'Confusion matrix + ROC curve',
          instructions: 'Read the confusion matrix on the left and the ROC curve on the right. Notice the values for accuracy, precision, recall, and F1.',
          expectedOutcome: 'Be able to interpret the confusion matrix and the AUC.',
        },
        quiz: [
          {
            id: 'pne-q1',
            question: 'In pneumonia diagnosis, which error is worse?',
            options: [
              'False positive (flag a healthy patient)',
              'False negative (miss a sick patient)',
              'They are equally bad.',
              'Neither — the model is perfect.',
            ],
            answerIndex: 1,
            explanation: 'A false negative means a sick patient is sent home — the cost is high.',
          },
          {
            id: 'pne-q2',
            question: 'AUC of 1.0 means:',
            options: ['Random', 'Perfect separation', 'Worse than random', 'Unrelated to model quality'],
            answerIndex: 1,
            explanation: 'AUC = 1.0 means the model perfectly ranks every positive above every negative.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, what is the precision of the model on the test set?',
          hint: 'Read the stat panel.',
          expectedAnswer: 'precision = TP / (TP + FP) — about 0.94 in the demo.',
        },
        keyTakeaways: [
          'Confusion matrix: TP, FP, FN, TN.',
          'Accuracy, precision, recall, F1, ROC AUC.',
          'In medical diagnosis, recall is more important than precision.',
        ],
      },
      {
        id: 'pneu-summary',
        number: 135,
        title: '135. Part VI Summary — Three Projects Side by Side',
        subtitle: 'Compare the three end-to-end projects — house price (regression), COVID-19 (time series), pneumonia (image classification).',
        pathId: 'data-science-6',
        pathTitle: 'Data Science VI · Projects',
        readingTime: '4 mins',
        difficulty: 'Beginner',
        objectives: [
          'Compare the three projects by effort, cost, interpretability, and complexity',
          'Name the best model from each project',
          'Connect each project to a real-world DS career skill',
        ],
        concepts: [
          {
            title: 'The three projects',
            paragraphs: [
              'Practical Data Science Projects: To consolidate your learning, this book includes several practical projects. These range from predicting house prices to customer segmentation, sentiment analysis, and more. These projects are designed to give you hands-on experience and a deeper understanding of how to apply data science theories in practice.',
            ],
          },
          {
            title: 'Project scoreboard',
            paragraphs: [
              'House Price Prediction: a regression problem on the Kaggle House Prices dataset. Random Forest beat SVR on the book\'s pipeline (MSE 857M vs 7.85B).',
              'COVID-19: a time-series problem on the OWID dataset. LSTM beat linear SVR (MAE 0.0025 vs 0.0045).',
              'Pneumonia diagnosis using CNN: an image classification problem on chest X-rays. Transfer learning with VGG16 — fine-tune the head, train for 10 epochs, plot train/val loss.',
            ],
          },
        ],
        glossary: [
          { term: 'Project scoreboard', definition: 'A one-glance summary of the best model and final error for each project.' },
        ],
        realWorldExample: {
          scenario: 'Building a DS portfolio',
          application: 'Each of the three projects exercises a different career skill: tabular regression (analyst), time-series (forecaster), image classification (CV engineer).',
          impact: 'Together they cover the three most common DS interview categories.',
        },
        playground: {
          type: 'pneu-summary',
          title: 'Three projects side by side',
          instructions: 'Read the radar chart on the left and the scoreboard on the right. Notice how each project trades off effort and complexity.',
          expectedOutcome: 'Be able to articulate which project is the right starting point for a new DS learner.',
        },
        quiz: [
          {
            id: 'psum-q1',
            question: 'Which project uses time-series modelling?',
            options: ['House Price', 'COVID-19', 'Pneumonia', 'All three'],
            answerIndex: 1,
            explanation: 'The COVID-19 project fits an LSTM on the daily OWID time series.',
          },
          {
            id: 'psum-q2',
            question: 'Which project uses transfer learning?',
            options: ['House Price', 'COVID-19', 'Pneumonia', 'None of them'],
            answerIndex: 2,
            explanation: 'The pneumonia CNN uses VGG16 pre-trained on ImageNet.',
          },
        ],
        practiceExercise: {
          task: 'In the simulator, which project has the highest model complexity?',
          hint: 'Read the radar chart.',
          expectedAnswer: 'Pneumonia — the VGG16-based CNN is the most complex model of the three.',
        },
        keyTakeaways: [
          'Three end-to-end projects covering regression, time series, and image classification.',
          'Each project is a different DS career skill: analyst, forecaster, CV engineer.',
          'Together they form a complete portfolio of beginner-friendly DS work.',
        ],
      },
    ],
  },
